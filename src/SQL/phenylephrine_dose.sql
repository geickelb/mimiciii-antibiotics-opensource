...
(
    select
    icustay_id, charttime, amountuom as amount_uom, rateuom as rate_uom
    ...
    --, max(case when itemid in (30127,30128) then amountuom else null end) as amount_uom  --these two lines provide units
	--, max(case when itemid in (30127,30128) then rateuom else null end) as rate_uom

  from mimiciii.inputevents_cv
  where itemid in (30127,30128) -- phenylephrine
  group by icustay_id, charttime, amount_uom, rate_uom
)
, vasocv2 as
(
  select v.*
